...
def main():
    ...
    min_passes = st.sidebar.slider("Minimum number of passes to show (highlights them grey)", 1, 10, 1)
    ...
    #passing_combinations, player_info = process_game_data(df_interact, team=selected_team)
    #G = create_passing_network(passing_combinations, player_info)
    G = process_data_into_graph(df_interact)
    #print(G.nodes)
    #print(G.edges)
    subG = create_player_subgraph(G, selected_team)
    subG2 = create_team_subgraph(G, selected_team)

    # Pyvis interactive
    html_path = create_pyvis_assist_network(subG, min_passes)


    #html_path = filter_down_and_create_assist_network(G, selected_team, min_passes)
    # Code for filtering dataframe and generating network
    st.subheader("Interactive Passing Network")
    with open(html_path, 'r', encoding='utf-8') as f:
        components.html(f.read(), height=600)    
    
    # debug_html = create_debug_html_component(subG)
    # st.subheader("Debug: Static Graph Visualization")
    # components.html(debug_html, height=650)


    # Passing Influence
    ranked_players = calculate_passing_influence(subG, alpha=0.9)
    #print(ranked_players)
    #print(type(ranked_players))
